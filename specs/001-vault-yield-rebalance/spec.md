...
1. **Given** vault metrics are older than the freshness limit, **When** a cycle runs, **Then** the bot does not trade and records a data-stale alert.
2. **Given** repeated transaction failures, **When** failures exceed the retry budget, **Then** the bot pauses automated execution until operator acknowledgment.
...
- How does the system handle partial transaction success (withdraw succeeds, deposit fails)?
...
- **FR-011**: System MUST reconcile actual wallet and vault positions before planning trades after any failed or partial execution.
- **FR-012**: System MUST skip trading when data freshness, connectivity, or vault availability checks fail, and record the reason.
...
- **Vault Metrics Snapshot**: Point-in-time measurements (yield, TVL, utilization, reserve breakdown, timestamp) used for scoring.
...
- **Rebalance Cycle**: One end-to-end evaluation from data fetch through decision to execution or skip.
...
- **SC-005**: Critical risk exits (vault risk score below floor) trigger reduction of exposure within one evaluation cycle when data is fresh and connectivity is healthy.
...
- Reliable access to Solana mainnet (RPC) and Kamino vault metric data
