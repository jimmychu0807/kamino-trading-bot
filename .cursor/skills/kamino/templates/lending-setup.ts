...
export class KaminoLendClient {
	...
	async initialize(): Promise<void> {
		...
		this.market = await KaminoMarket.load(this.connection, CONFIG.mainMarket, CONFIG.commitment);
	...
	private async sendAction(action: KaminoAction): Promise<string> {
		const instructions = [...action.setupIxs, ...action.lendingIxs, ...action.cleanupIxs];
