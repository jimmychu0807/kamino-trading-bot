...
export class KaminoClient {
	...
	async withdraw(symbol: string, amount: number | "max"): Promise<string> {
		...
		const withdrawAmount =
			amount === "max"
				? "max"
				: new Decimal(amount).mul(new Decimal(10).pow(reserve.stats.decimals)).floor().toString();
	...
	async getLiquidityPositions() {
		...
		for (const { address, strategy } of strategies) {
			...
			const balance = await this.liquidity.getTokenAccountBalanceOrZero(kTokenAta.address);
	...
	async withdrawFromStrategy(
		...
		let withdrawIxs: any;
		if (shares === "all") {
			withdrawIxs = await this.liquidity.withdrawAllShares(strategyAddress, this.address, slippage);
		} else {
			withdrawIxs = await this.liquidity.withdraw(strategyAddress, this.address, shares, slippage);
		}
	...
	async getPortfolioSummary() {
		const _prices = await this.getAllPrices();
		...
		return {
			...
			lending: {
				...
				netValue: lendingPosition.totalDeposited.sub(lendingPosition.totalBorrowed),
